#!/usr/bin/env bash
# =========================================================================
# ⚠️  TEMPORARY COMPATIBILITY COPY — this script has MOVED to db-access/forward.sh
#     (alongside the new db-login.sh / pg-token.sh). This copy exists only so that
#     existing aliases pointing at database-forwarder/forward.sh keep working for
#     now; it is kept identical to db-access/forward.sh. Please update your alias to
#     point at db-access/forward.sh. This folder is slated for removal after the
#     summer (≈ 2026-08). For a DB access token, use db-login.sh in db-access/.
# =========================================================================
