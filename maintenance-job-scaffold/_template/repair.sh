#!/usr/bin/env bash
#
# {{JOB_KEBAB}}: drive maintenance.{{JOB_LOWER}}_run_batch in a loop until the
...
export PGAPPNAME="maint-{{JOB_KEBAB}}-${WORKER_ID}"
...
while :; do
    ...
    result=$(psql "${PSQL_ARGS[@]}" -c "
        ...
        CALL maintenance.{{JOB_LOWER}}_run_batch(
