-- iss1712-dpcleanup-actors: at-a-glance progress. Safe to run repeatedly while workers are
...
SELECT
    ...
    END AS eta
FROM maintenance."Iss1712DpcleanupActors_Progress" p
CROSS JOIN LATERAL (
    ...
    SELECT SUM("Claimed")::numeric
           ...
           AS rows_per_sec
    FROM maintenance."Iss1712DpcleanupActors_BatchLog"
...
\echo '-- repair.sh sets application_name = maint-iss1712-dpcleanup-actors-<WORKER_ID>.'
SELECT
    application_name,
    state,
    wait_event_type,
...
WHERE application_name LIKE 'maint-iss1712-dpcleanup-actors-%'
