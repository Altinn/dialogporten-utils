...
def read_party_services(path: str) -> list[dict]:
    try:
        with open(path, "r", encoding="utf-8") as handle:
    ...
    if not isinstance(data, list) or not data:
        raise SystemExit(f"File must contain a non-empty JSON array: {path}")

    pool = []
    for index, entry in enumerate(data, start=1):
        if not isinstance(entry, dict):
            raise SystemExit(f"Invalid party/service entry #{index}: expected object")
        party = entry.get("Party")
        services = entry.get("Services")
        if not isinstance(party, str) or not party:
            raise SystemExit(f"Invalid party/service entry #{index}: missing Party")
        if not isinstance(services, list) or not all(isinstance(s, str) and s for s in services):
            raise SystemExit(f"Invalid party/service entry #{index}: Services must be non-empty strings")
        if not services:
            warn(f"Skipping party/service entry #{index} with no services: {party}")
            continue
        pool.append(
            {
                "Party": party,
                "Services": sorted(set(services)),
                "EstimatedDialogCount": entry.get("EstimatedDialogCount"),
            }
        )

    if not pool:
        raise SystemExit(f"No party/service entries with services found in: {path}")
    return pool
...
def generate_correlated_case(
    rng: random.Random,
    party_services_pool: list[dict],
    total_parties: int,
    total_services: int,
    group_count: int,
) -> list[dict]:
    if total_parties < 1:
        raise SystemExit("Total party count must be >= 1.")
    if group_count < 1:
        raise SystemExit("Group count must be >= 1.")
    if group_count != total_parties:
        raise SystemExit(
            "Correlated party/service mode requires groups to equal parties "
            "(one permission group per party)."
        )

    selected = rng.sample(party_services_pool, total_parties)
    case = []
    for entry in selected:
        services = list(entry["Services"])
        if total_services > 0:
            service_count = min(total_services, len(services))
            services = rng.sample(services, service_count)
            services.sort()
        case.append({"Parties": [entry["Party"]], "Services": services})
    return case
...
def main() -> None:
    ...
    parser.add_argument(
        "--party-services-path",
        help="Path to correlated party/service JSON pool generated by generate_samples.py party-services",
    )
    ...
    correlated_mode = args.party_services_path is not None
    if correlated_mode:
        party_services_pool = read_party_services(args.party_services_path)
        party_pool = [entry["Party"] for entry in party_services_pool]
        service_pool = sorted({service for entry in party_services_pool for service in entry["Services"]})
    else:
        party_services_pool = []
