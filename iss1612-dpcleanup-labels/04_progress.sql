-- iss1612-dpcleanup-labels: at-a-glance progress. Safe to run repeatedly while workers are
...
SELECT
    ...
    END AS eta
FROM maintenance."Iss1612DpcleanupLabels_Progress" p
CROSS JOIN LATERAL (
    ...
    SELECT SUM("Claimed")::numeric
           ...
           AS rows_per_sec
    FROM maintenance."Iss1612DpcleanupLabels_BatchLog"
...
\echo '-- repair.sh sets application_name = maint-iss1612-dpcleanup-labels-<WORKER_ID>.'
SELECT
    application_name,
    state,
    wait_event_type,
...
WHERE application_name LIKE 'maint-iss1612-dpcleanup-labels-%'
