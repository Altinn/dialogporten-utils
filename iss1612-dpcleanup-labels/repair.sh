#!/usr/bin/env bash
#
# iss1612-dpcleanup-labels: drive maintenance.iss1612dpcleanuplabels_run_batch in a loop until the
...
export PGAPPNAME="maint-iss1612-dpcleanup-labels-${WORKER_ID}"
...
cat >&2 <<BANNER
$(ts) iss1612-dpcleanup-labels worker starting
...
while :; do
    ...
    result=$(psql "${PSQL_ARGS[@]}" -c "
        ...
        CALL maintenance.iss1612dpcleanuplabels_run_batch(
